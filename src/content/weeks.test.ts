...
describe('week registry', () => {
  ...
  it('publishes Weeks 02, 05, and 06 and reserves the remaining routes', () => {
    expect(WEEKS.filter((week) => week.status === 'published').map((week) => week.id)).toEqual(['02', '05', '06'])
    expect(WEEKS.filter((week) => week.status === 'placeholder').map((week) => week.id)).toEqual(['01', '03', '04'])
  })
